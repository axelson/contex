defmodule Contex.BarChart do
  ...
  def force_value_range(%BarChart{} = plot, {min, max} = value_range)
  ...
  def to_svg(
    ...
    cat_axis_svg =
      if options.show_cat_axis do
        Axis.to_svg(category_axis)
      else
        ""
      end

    val_axis_svg =
      if options.show_val_axis do
        Axis.to_svg(value_axis)
      else
        ""
      end
  ...
  defp get_category_axis(category_scale, _, plot) do
    ...
    category_axis =
      case length(Scale.ticks_range(category_scale)) > 8 do
  ...
  defp get_svg_bars(%BarChart{dataset: dataset} = plot) do
    ...
    fills =
      Enum.map(plot.value_cols, fn column ->
  ...
  defp get_svg_bar(
         row,
         %BarChart{category_scale: category_scale, value_scale: value_scale} = plot,
         cat_col_index,
         val_col_indices,
         fills
       ) do
  ...
  defp get_bar_event_handlers(
         %BarChart{phx_event_handler: phx_event_handler, value_cols: value_cols},
         category,
         series_values
       )
  ...
  defp get_bar_event_handlers(%BarChart{value_cols: value_cols}, _, _),
    do: Enum.map(value_cols, fn _ -> [] end)
  ...
  defp get_bar_opacities(%BarChart{value_cols: value_cols}, _),
    do: Enum.map(value_cols, fn _ -> "" end)
    ...
    texts =
      case count < 4 and plot.data_labels do
        ...
        _ ->
          ...
          |> Enum.with_index()
          |> Enum.map(fn {{bar_value, label, adjusted_band}, index} ->
            get_svg_bar_label(plot.orientation, bar_value, label, adjusted_band, plot, index)
          end)
      end
  ...
  defp get_svg_bar_label(:horizontal, {_, bar_end} = bar, label, cat_band, _plot, _index) do
  ...
  defp get_svg_bar_label(_, {bar_start, _} = bar, label, cat_band, _plot, index) do
  ...
  def set_val_col_names(%BarChart{} = plot, val_col_names) when is_list(val_col_names) do
    case Dataset.check_column_names(plot.dataset, val_col_names) do
      {:ok, []} ->
        ...
        %{
          plot
          | value_cols: val_col_names,
            value_scale: val_scale,
            series_fill_colours: series_fill_colours
        }
